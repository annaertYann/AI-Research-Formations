...
{
	HitBox();
	explicit HitBox(float horizontalShift, float VerticalShift, float widthDiff, float heightDiff);
...
struct AttackBox
{
	AttackBox(Faction f, Rectf coll, float lifetime,float dam,AdvancedGameObject* linkk,float speedx,float speedy)
	{
		faction = f;
		collider = coll;
		lifeTime = lifetime;
	...
	BasicGameObject* renderer = nullptr;
	AdvancedGameObject* link;
	Faction faction;
	Rectf collider;
	float accuSec=0;
	float lifeTime;
